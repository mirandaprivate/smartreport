...
import (
	...
	"logi/rules/plugin/templates/base/validate/validate"
)

// This file is copy from
// github.com/envoyproxy/protoc-gen-validate/module/checker.go.
// It aims to check the validate rules.
...
func (m *pluginModule) CheckRules(msg pgs.Message) {
...
func (m *pluginModule) CheckFieldRules(
	typ FieldType,
...
func (m *pluginModule) MustType(
	typ FieldType,
	pt pgs.ProtoType,
	wrapper pgs.WellKnownType,
) {
	if emb := typ.Embed(); emb != nil && emb.IsWellKnown() &&
		emb.WellKnownType() == wrapper {
...
func (m *pluginModule) CheckFloat(r *validate.FloatRules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckDouble(r *validate.DoubleRules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckInt32(r *validate.Int32Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckInt64(r *validate.Int64Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckUInt32(r *validate.UInt32Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckUInt64(r *validate.UInt64Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckSInt32(r *validate.SInt32Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckSInt64(r *validate.SInt64Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckFixed32(r *validate.Fixed32Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckFixed64(r *validate.Fixed64Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckSFixed32(r *validate.SFixed32Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckSFixed64(r *validate.SFixed64Rules) {
	m.checkNums(len(r.In), len(r.NotIn), r.Const, r.Lt, r.Lte, r.Gt, r.Gte)
}

func (m *pluginModule) CheckString(r *validate.StringRules) {
	...
	if r.MaxLen != nil {
		max := int(r.GetMaxLen())
		m.Assert(utf8.RuneCountInString(
			r.GetPrefix()) <= max, "`prefix` length exceeds the `max_len`",
		)
		m.Assert(utf8.RuneCountInString(
			r.GetSuffix()) <= max, "`suffix` length exceeds the `max_len`",
		)
		m.Assert(utf8.RuneCountInString(
			r.GetContains()) <= max, "`contains` length exceeds the `max_len`",
		)
	...
	if r.MaxBytes != nil {
		...
		m.Assert(
			len(r.GetPrefix()) <= max,
			"`prefix` length exceeds the `max_bytes`",
		)
		m.Assert(
			len(r.GetSuffix()) <= max,
			"`suffix` length exceeds the `max_bytes`",
		)
		m.Assert(
			len(r.GetContains()) <= max,
			"`contains` length exceeds the `max_bytes`",
		)
	}
}

func (m *pluginModule) CheckBytes(r *validate.BytesRules) {
	m.checkMinMax(r.MinLen, r.MaxLen)
	m.checkIns(len(r.In), len(r.NotIn))
	m.checkPattern(r.Pattern, len(r.In))
	...
	if r.MaxLen != nil {
		max := int(r.GetMaxLen())
		m.Assert(
			len(r.GetPrefix()) <= max, "`prefix` length exceeds the `max_len`",
		)
		m.Assert(
			len(r.GetSuffix()) <= max, "`suffix` length exceeds the `max_len`",
		)
		m.Assert(
			len(r.GetContains()) <= max,
			"`contains` length exceeds the `max_len`",
		)
	}
}

func (m *pluginModule) CheckEnum(ft FieldType, r *validate.EnumRules) {
	m.checkIns(len(r.In), len(r.NotIn))
	...
	if r.GetDefinedOnly() && len(r.In) > 0 {
		...
		for _, in := range r.In {
			if _, ok = vals[in]; !ok {
				m.Failf("undefined `in` value (%d) conflicts with " +
					"`defined_only` rule")
			}
		}
	}
...
func (m *pluginModule) CheckMessage(f pgs.Field, rules *validate.FieldRules) {
...
func (m *pluginModule) CheckRepeated(ft FieldType, r *validate.RepeatedRules) {
...
func (m *pluginModule) CheckMap(ft FieldType, r *validate.MapRules) {
	typ := m.mustFieldType(ft)
...
func (m *pluginModule) CheckAny(_ FieldType, r *validate.AnyRules) {
	m.checkIns(len(r.In), len(r.NotIn))
}

func (m *pluginModule) CheckDuration(_ FieldType, r *validate.DurationRules) {
	m.checkNums(
		len(r.GetIn()),
		len(r.GetNotIn()),
...
func (m *pluginModule) CheckTimestamp(
	_ FieldType,
	r *validate.TimestampRules,
) {
	m.checkNums(0, 0,
	...
	m.Assert(
		(r.LtNow == nil && r.GtNow == nil) ||
			(r.Lt == nil && r.Lte == nil && r.Gt == nil && r.Gte == nil),
...
func (m *pluginModule) mustFieldType(ft FieldType) pgs.FieldType {
...
func (m *pluginModule) checkNums(
	in, notIn int, ci, lti, ltei, gti, gtei interface{},
) {
	m.checkIns(in, notIn)
...
func (m *pluginModule) checkIns(in, notIn int) {
	m.Assert(
		in == 0 || notIn == 0,
		"cannot have both `in` and `not_in` rules on the same field")
}

func (m *pluginModule) checkMinMax(min, max *uint64) {
	if min == nil || max == nil {
		return
	}
...
func (m *pluginModule) checkLen(len, min, max *uint64) {
	if len == nil {
		return
	}
...
func (m *pluginModule) checkPattern(p *string, in int) {
	if p != nil {
...
func (m *pluginModule) checkDur(d *duration.Duration) *time.Duration {
	if d == nil {
		return nil
	}
...
func (m *pluginModule) checkTS(ts *timestamp.Timestamp) *int64 {
	if ts == nil {
		return nil
	}
